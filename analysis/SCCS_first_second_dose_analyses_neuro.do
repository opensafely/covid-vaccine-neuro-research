...
replace censor_fu_dose2=1 if second_pfizer_date == second_moderna_date & second_pfizer_date != . & censor_fu_dose2!=1  & (first_brand=="PF" | first_brand=="MOD")
...
*count and create flags for when second dose brand is different to first
 ...
 display "AZ"
 count if ((first_brand=="AZ" & second_pfizer_date!=.) | (first_brand=="AZ" & second_moderna_date!=.)) & first_brand=="AZ"
 display "PF"
 count if ((first_brand=="PF" & second_az_date!=.) | (first_brand=="PF" & second_moderna_date!=.)) & first_brand=="PF"
 display "MOD"
 count if ((first_brand=="MOD" & second_pfizer_date!=.) | (first_brand=="MOD" & second_az_date!=.)) & first_brand=="MOD"
 
 
gen censor_fu_diff_brand2=1 if ((first_brand=="AZ" & second_pfizer_date!=.) | (first_brand=="AZ" & second_moderna_date!=.)) & first_brand=="AZ"
replace censor_fu_diff_brand2=1 if ((first_brand=="PF" & second_az_date!=.) | (first_brand=="PF" & second_moderna_date!=.)) & first_brand=="PF"
replace censor_fu_diff_brand2=1 if ((first_brand=="MOD" & second_pfizer_date!=.) | (first_brand=="MOD" & second_az_date!=.)) & first_brand=="MOD"
...
count if censor_fu_diff_brand2==1 & censor_fu_dose2!=1 & censor_fu_dose2!=.
...
*flag to include 2nd dose if not 2 different on same day, not differnet brand to 1st...
...
*is it right that there will only be one date in any of the second dose varaibles? i.e. same date in second_az_dateand second_any_vaccine_date if second dose is AZ, etc.?
...
replace cutp3= vacc_date1-14 if outcome=="BP"
...
gen cutp12=vacc_date2+42 



*need to consider that risk window after dose 1 is censored at vacc date2 +4 days

*** for those with risk window after dose 1 overlapping with day 4-42 after dose 2, censor risk window post dose 1 at dose 2 +4 days
	...
	*1. create variable including second dose risk windows for BP, TM and for GBS
		...
		recode exgr1 (0=0) (1=1) (2=2) (3=3) (4=4) (5=0) (6=5) (7=6) (8=7) (9=8), generate(vacc1_`j'_incl_dose2)
			** vacc1_`j'_dose2 has 9 levels, non-risk (0), pre-vacc low (1), day 0 (2) days 1-3 (3), days 4-28/42 (4),  pre-vacc low dose 2 (5), day 0 dose2 (6) days 1-3 dose2 (7), days 4-42 dose 2 (8)
			label define vacc1_`j'_incl_dose21 0 "non-risk" 1 "pre-vacc" 2 "day 0" 3 "days 1-3" 4 "days 4-28 or 42"  5 "pre-vacc dose2" 6 "day 0 dose2" 7 "days 1-3 dose 2" 8 "days 4-42 dose2"
...
** vacc1_`j'_dose2 has 8 levels, non-risk (0), pre-vacc low (1), day 0 (2) days 1-3 (3), days 4-28/42 (4),  pre-vacc low dose 2 (5), day 0 dose2 (6) days 1-3 dose2 (7), days 4-42 dose 2 (8)
 ...
 xtpoisson nevents ib0.vacc1_`j'_incl_dose2  , fe i(patient_id) offset(loginterval) eform

 
 display "add in week"
 
  xtpoisson nevents ib0.vacc1_`j'_incl_dose2 ib0.week , fe i(patient_id) offset(loginterval) eform
 
 display "add in 2 week period"
 
 xtpoisson nevents ib0.vacc1_`j'_incl_dose2 ib0.two_week , fe i(patient_id) offset(loginterval) eform
